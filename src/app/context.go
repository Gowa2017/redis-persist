...
func procs(ud interface{}, args []string) (result string, err error) {
	...
	if len(args) > 0 {
		if count, err = strconv.Atoi(args[0]); err != nil {
			log.Printf("[ERROR]illegal parameter: %v", err)
			return
		}
	}
...
func info(ud interface{}, args []string) (result string, err error) {
	...
	prop, _ := db.GetProperty(property)
	if prop == "" {
...
func sync_all(ud interface{}, args []string) (result string, err error) {
	...
	if err != nil {
		log.Printf("[ERROR]sync_all cmd service failed:%v", err)
		return
	}
...
func check(ud interface{}, args []string) (result string, err error) {
	...
	for i, key := range keys {
		...
		if err != nil {
			log.Printf("[ERROR]hgetall key %s failed:%v", key, err)
			return
		}
		var chunk []byte
		var leveldb_data map[string]string
		if chunk, err = db.Get([]byte(key), nil); err != nil {
			log.Printf("[ERROR]leveldb.Get failed on key %s failed:%v", key, err)
			return
		}
		if chunk == nil {
			if miss != nil {
		...
		if err != nil {
			log.Printf("[ERROR]json.Unmarshal failed on key %s failed:%v", key, err)
			continue
		}
...
func fast_check(ud interface{}, args []string) (result string, err error) {
	...
	for i, key := range keys {
		...
		if bak_version, err = db.Get([]byte(index_key), nil); err != nil {
			return
		}
		if bak_version == nil {
			if miss != nil {
...
func dump(ud interface{}, args []string) (result string, err error) {
	...
	chunk, err := db.Get([]byte(key), nil)
	if chunk == nil || err != nil {
		log.Printf("[ERROR]fetch data failed:%v", err)
		return
	}
	...
	if err != nil {
		log.Printf("[ERROR]unmarshal chunk failed:%v", err)
		return
	}
...
func restore(ud interface{}, key string, cli *redis.Redis) (err error) {
	...
	if redis_data["version"] >= leveldb_data["version"] && len(redis_data) > 0 {
		log.Printf("redis_data[version]:%s >= leveldb_data[version]:%s", redis_data["version"], leveldb_data["version"])
		return
	}
	...
	if err != nil {
		log.Printf("[ERROR]hmset key %s failed:%v", key, err)
		return
	}
	return
}
...
func keys(ud interface{}, args []string) (result string, err error) {
	...
	if len(args) > 0 {
		if start, err = strconv.Atoi(args[0]); err != nil {
			log.Printf("[ERROR]iter start error: %v", err)
			return
		}
	}
...
func diff(ud interface{}, args []string) (result string, err error) {
	...
	chunk, err := db.Get([]byte(key), nil)
	if chunk == nil || err != nil {
		log.Printf("[ERROR]fetch data from leveldb failed:%v", err)
		return
	}
